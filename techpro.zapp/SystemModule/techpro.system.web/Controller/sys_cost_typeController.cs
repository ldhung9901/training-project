...
{
    public partial class sys_cost_typeController : BaseAuthenticationController
    {
        private sys_cost_type_repo repo;

        public sys_cost_typeController(IUserService UserService,techproDefautContext context) : base(UserService)
        {
            repo = new sys_cost_type_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_cost_types
                .Where(d=>d.status_del==1)
                 .Select(d => new
                 {
                     id = d.id,
                     name = d.name,
                     cost_type_code=d.cost_type_code
                 }).ToList();
            return Json(result);
        }
        [HttpPost]
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_cost_type_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<sys_cost_type_model> result = new DTResult<sys_cost_type_model>
                {
