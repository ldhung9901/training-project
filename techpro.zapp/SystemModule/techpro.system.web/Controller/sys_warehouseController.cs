...
{
    public partial class sys_warehouseController : BaseAuthenticationController
    {
        public sys_warehouse_repo repo;

        public sys_warehouseController(IUserService UserService, techproDefautContext context) : base(UserService)
        {
            repo = new sys_warehouse_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_warehouses.Where(d => d.status_del == 1).
                 Select(d => new
                 {
                     id = d.id,
                     name = d.name
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_warehouse_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var query = repo.FindAll()
                     ...
                     .Where(d => d.db.name.Contains(search) || d.db.location.Contains(search)|| d.db.note.Contains(search))
                     ;
                ...
                DTResult<sys_warehouse_model> result = new DTResult<sys_warehouse_model>
                {
