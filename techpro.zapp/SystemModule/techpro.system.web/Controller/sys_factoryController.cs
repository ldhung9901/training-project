...
{
    public partial class sys_factoryController : BaseAuthenticationController
    {
        public sys_factory_repo repo;

        public sys_factoryController(IUserService UserService, techproDefautContext context) : base(UserService)
        {
            repo = new sys_factory_repo(context);
        }

        public IActionResult getListUse()
        {
            var result = repo._context.sys_factorys.Where(d => d.status_del == 1).
                 Select(d => new
                 {
                     id = d.id,
                     name = d.name
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_factory_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                DTResult<sys_factory_model> result = new DTResult<sys_factory_model>
                {
