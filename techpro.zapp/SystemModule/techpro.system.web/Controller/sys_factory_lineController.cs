...
{
    public partial class sys_factory_lineController : BaseAuthenticationController
    {
        private sys_factory_line_repo repo;

        public sys_factory_lineController(IUserService UserService, techproDefautContext context) : base(UserService)
        {
            repo = new sys_factory_line_repo(context);
        }
        

        public IActionResult getListUses([FromBody] JObject json)
        {
            
            var result = repo._context.sys_factory_lines
        ...
        public IActionResult getListUse([FromBody] JObject json)
        {
            ...
            var result = repo._context.sys_factory_lines
                .Where(t => t.id_factory == id_factory)
                  .Where(t => t.status_del == 1)
                 .Select(d => new
                 {
                     id = d.id,
        ...
        public async Task<IActionResult> create([FromBody] JObject json)
        {
            ...
            var model = JsonConvert.DeserializeObject<sys_factory_line_model>(json.GetValue("data").ToString());
        ...
        public async Task<IActionResult> DataHandler([FromBody] JObject json)
        {
            try
            {
                ...
                var id_factory = dictionary["id_factory"];
                var search = dictionary["search"];
                var query = repo.FindAll()
                    //.Where(d=>d.db.status_del==1)
                    .Where(d=>d.db.id_factory == id_factory)
                ...
                DTResult<sys_factory_line_model> result = new DTResult<sys_factory_line_model>
                {
